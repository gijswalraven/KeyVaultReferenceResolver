...
namespace KeyVaultReferenceResolver
{
    ...
    internal sealed class ResolvedSecretsSource : IConfigurationSource
    {
        ...
        /// reference in one reaches the application as the literal <c>@Microsoft.KeyVault(...)</c> string and is used as
        ...
        /// error, and that case is still reported as Log.ResolverNotLastSource at
        ...
        private void InspectLaterSources(IConfigurationBuilder builder)
        {
            ...
            catch (Exception)
            {
                ...
                Log.ResolverNotLastSource(_logger, later.Count);
                return;
            }
            ...
            foreach (var key in unresolved)
                Log.UnresolvedReference(_logger, key);

            throw new KeyVaultReferenceResolutionException(
                "Configuration source(s) registered after AddKeyVaultReferenceResolver carry unresolved reference(s) for: " +
                ...
                "AddKeyVaultReferenceResolver after every other configuration source.");
        }
        ...
        private static List<string> FindReferences(List<IConfigurationSource> sources, IConfigurationBuilder builder)
        {
            ...
            try
            {
                ...
                {
                    if (!string.IsNullOrEmpty(entry.Value) && KeyVaultReferenceResolverExtensions.IsKeyVaultReference(entry.Value))
                        found.Add(entry.Key);
                }
